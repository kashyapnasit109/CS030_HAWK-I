...
2. Extracts real FPS dynamically via cv2.VideoCapture(CAP_PROP_FPS)
...
async def detect_velocity(
  ...
  # ── Step 2: Save Temporary Video File ────────────────────────────────
  suffix = os.path.splitext(file.filename or "clip.mp4")[1]
  if not suffix:
    suffix = ".mp4"
  ...
  try:
    ...
    if not fps or fps <= 0 or math.isnan(fps):
      logger.warning("Dynamic FPS extraction returned invalid value. Falling back to 30.0 FPS.")
      fps = 30.0
    else:
      logger.info(f"Extracted dynamic FPS: {fps:.2f}")

    total_frames = int(cap.get(cv2.CAP_PROP_FRAME_COUNT))
    cap.release()

    # ── Step 4: Run YOLOv8 ByteTrack ───────────────────────────────────
    results = yolo.track(
    ...
    # ── Step 5: Compute Velocity per Tracked Object ────────────────────
    tracked_output = []
